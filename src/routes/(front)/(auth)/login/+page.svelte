	...
	<div class="card w-96 bg-neutral text-neutral-content mx-auto my-20">
		<div class="card-body">
			...
			<form class="form-control gap-2" method="POST" use:enhance on:submit={() => loading = true}>
				...
				{/if}
				<button type="submit" class="btn btn-primary w-full my-4" disabled={loading}>
					{#if loading}
			...
			<div>
				<a class="link text-sm" href="/login/forgot-password{email != '' ? '?email=' + encodeURIComponent(email) : ''}">Forgot password?</a>
