...
<svelte:head>
	...
	<meta
		name="description"
		content="Discover the many ways to use custom AI models, new features and ways to better implement your chatbots."
	/>
</svelte:head>
...
<div class="container grid md:grid-cols-2 lg:grid-cols-3 gap-10">
	{#each data.posts as post, i}
		<a class="card card-compact bg-neutral overflow-hidden group" href="/blog/{post.slug}">
			...
			{:then postImages}
				<img
					src={postImages[i].thumbnail}
					width="666"
					height="480"
					alt=""
					class="group-hover:scale-105 origin-bottom transition-transform"
					in:fade
				/>
			{/await}
			<div class="card-body">
				<div class="flex justify-between gap-2">
					...
					<span class="text-sm font-light">
						{new Date(post._modified * 1000).toLocaleDateString()}
					</span>
				</div>
