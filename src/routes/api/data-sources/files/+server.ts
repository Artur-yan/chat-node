...
const session = await locals.auth.validate();
  ...
  console.log(form_tmp)
    ...
    const response = await fetch(`https://api.carbon.ai/uploadfile?chunk_size=${chunkSize}&chunk_overlap=${chunkOverlap}&skip_embedding_generation=false&embedding_model=OPENAI_ADA_LARGE_3072&useOCR=${use_ocr}`, options)
