...
<div class="relative">
  ...
  {/if}
  <div class="flex gap-1 {settings.suggestedQuestionsStacked ? 'flex-wrap' : 'overflow-x-auto'} mb-2 w-full px-2">
    {#each suggestedQuestions as question, i}
      <button
        ...
        class="btn btn-sm text-xs rounded-md normal-case bg-[var(--userBubbleBG)] text-[var(--userBubbleText)] border-[var(--suggestedQuestionBorder)] hover:bg-[var(--botBubbleBG)] hover:text-[var(--botBubbleText)]"
        type="button"
      >
        {question.label}
      </button>
